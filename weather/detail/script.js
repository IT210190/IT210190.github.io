    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            document.getElementById('erg').innerHTML = '<h2 class="date" onclick="changeSite()">' + date.toString().split(" ")[0] + ' ' + (erg.list[startingNumber].dt_txt).split(" ")[0] + '</h2>' +
                ...
                '<div class="littleContainer"><h3>' + erg.list[startingNumber].weather[0].main + '</h3><img src="https://openweathermap.org/img/wn/' + erg.list[startingNumber].weather[0].icon + '@2x.png" alt="weatherIcon" class="img"><p>' + erg.list[startingNumber].weather[0].description + '</p></div>' +
            ...
            for (let i = 0; i < 24; ++i) {
                document.getElementById('hourlyDetails').innerHTML += '<div class="hourlyHours"><h4>' + (erg.list[startingNumber + i].dt_txt).split(" ")[1].split(":")[0] + 'h</h4><p>' + erg.list[startingNumber + i].main.temp + '°C</p><p>Regen: ' + (erg.list[startingNumber + i].pop * 100).toFixed(2) + '%</p><img src="https://openweathermap.org/img/wn/' + erg.list[startingNumber + i].weather[0].icon + '@2x.png" alt="weatherIcon" class="img"></div>';
            }
        }
    };
